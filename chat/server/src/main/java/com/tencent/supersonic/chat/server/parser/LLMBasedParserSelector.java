...
public class LLMBasedParserSelector {
    ...
    public static final String PROMPT_TEMPLATE =
                "你是一个语义理解助手，负责根据用户的自然语言问题选择最合适的解析器。\n" +
                        ...
                        "3、功能调用/专业知识询问识别机制：\n" +
                        "若问题包含动作性动词（如\"发送/生成/执行...\"）或需要外部系统交互（如API调用、文件操作），或涉及非结构化数据询问（如专业知识询问、规章制度询问、业务口径询问、数据库表结构询问等），则触发 NL2Plugin解析器 。例如：\"查询销售数据的表结构\"。\n"+
                        "4、歧义处理策略\n" +
                        "当意图模糊时，默认使用NL2SQL解析器。\n" +
                        "5、当用户指定解析器时，强制使用用户指定的解析器。\n"+
                        "6、根据历史问题，判断当前问题是否是多轮对话改写问题，如果是，强制使用NL2SQL解析器。\n"+
